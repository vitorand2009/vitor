...
import { History, Star, Clock, User, Users, Search, Trash2, FileText } from "lucide-react"
...
export default function HistoricoPage() {
  ...
  const fetchDegustacoes = async () => {
    try {
      ...
      const finalizadas = data.filter((deg: Degustacao) => deg.status === "finalizada")
  ...
  const filteredDegustacoes = degustacoes.filter(
    (degustacao) =>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold text-gray-800">Histórico de Degustações</h1>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {filteredDegustacoes.map((degustacao) => (
          <Card key={degustacao.id} className="bg-white shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <CardTitle className="text-xl mb-1">{degustacao.charuto?.nome || "Charuto não encontrado"}</CardTitle>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {degustacao.nota && (
                    <div className="flex items-center gap-1 bg-amber-100 px-3 py-1 rounded-full">
                      ...
                      <span className="font-medium text-amber-800">{degustacao.nota}/10</span>
                    </div>
                  )}
                  <Button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                {degustacao.foto_anilha && (
                  <div className="space-y-2">
                    ...
                    <img
                      src={degustacao.foto_anilha || "/placeholder.svg"}
                      alt="Anilha"
              ...
              {degustacao.folha_anilhada && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">{degustacao.folha_anilhada}</p>
                </div>
              )}
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {getSaboresAtivos(degustacao).map((sabor) => (
                      <span key={sabor} className="bg-amber-100 text-amber-800 px-2 py-1 rounded-full text-sm">
                        {sabor}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  {degustacao.observacoes && (
                    <div>
                      ...
                      <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">{degustacao.observacoes}</p>
                    </div>
                  )}
                  {degustacao.observacao_anilha && (
                    <div>
                      ...
                      <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">{degustacao.observacao_anilha}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      {filteredDegustacoes.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            {searchTerm ? "Tente ajustar sua busca" : "Complete algumas degustações para ver o histórico aqui"}
          </p>
        </div>
      )}
    </div>
